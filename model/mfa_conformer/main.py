...
import os
import sys
egs_path = os.path.dirname(os.path.abspath(os.path.abspath(__file__)))
...
class Task(LightningModule):
    def __init__(
        ...
        **kwargs
        ...
        if os.path.exists(self.hparams.trial_path):
    ...
    def extrac_embeddings(self, signal_data, sampling_rate, signal_start_time, window_len=3.0, hop_size=1.0, device=torch.device("cpu"), pad_signal=False):
    # signal_data: [num_channels, num_samples]
    # if the signal is multi-channel, the mean emb will be returned

        segments = []
        embs = []
        hop_samples = int(hop_size * sampling_rate)
        window_samples = int(window_len * sampling_rate)

        signal_len = signal_data.shape[-1]
        
        if pad_signal and signal_len < window_samples:
            signal_data = torch.nn.functional.pad(signal_data, (0, window_samples-signal_len), mode='replicate')
        
        for idx_start in range(0, signal_len, hop_samples):
            window_sig = signal_data[:, idx_start:idx_start+window_samples].to(device)
            seg_len = window_sig.shape[-1]
            try:
                with torch.no_grad():
                    emb = self.forward(window_sig)
                    if emb.shape[0] > 1:
                    ...
                    seg_start = idx_start / sampling_rate + signal_start_time
                    seg_end = seg_start + seg_len / sampling_rate
